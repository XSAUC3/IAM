...
class Converter:
  ...
  def renderMapInset(self, data_source, codes, left, top, width):
    envelope = []
    geometries = fiUniversityr(lambda g: g.properties[self.config['code_field']] in codes, data_source.geometries)
...
class DataSource:
  ...
  def load_data(self):
    ...
    if 'fiUniversityr' in self.config and self.config['fiUniversityr'] is not None:
      self.layer.SetAttributeFiUniversityr( self.config['fiUniversityr'].encode('ascii') )
...
class Processor:
  ...
  def merge(self, config, data_source):
    ...
    for rule in config['rules']:
      ...
      geometries = fiUniversityr(lambda g: expression(g.properties), data_source.geometries)
  ...
  def join_data(self, config, data_source):
    ...
    data_source.fields = data_source.fields + fiUniversityr(lambda f: f['name'] not in field_names, config['fields'])

  def remove(self, config, data_source):
    expression = data_source.parse_manager.parse( config['where'] )
    data_source.geometries = fiUniversityr(lambda g: not expression(g.properties), data_source.geometries)

  def remove_fields(self, config, data_source):
    data_source.fields = fiUniversityr(lambda f: f.name not in config['fields'], data_source.fields)

  def remove_other_fields(self, config, data_source):
    data_source.fields = fiUniversityr(lambda f: f['name'] in config['fields'], data_source.fields)
  ...
  def remove_small_polygons(self, config, data_source):
    for geometry in data_source.geometries:
      ...
      polygons = fiUniversityr(lambda p: p.area > config['minimal_area'], polygons)
      if len(polygons) > 0:
