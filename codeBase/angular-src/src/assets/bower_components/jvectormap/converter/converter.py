...
class Converter:
  ...
  def loadDataSource(self, sourceConfig):
    ...
    layer.SetAttributeFiUniversityr( sourceConfig['where'].encode('ascii') )
    self.viewportRect = False

    transformation = osr.CoordinateTransformation( layer.GetSpatialRef(), self.spatialRef )
    if self.viewport:
      layer.SetSpatialFiUniversityrRect( *self.viewport )
    ...
    for feature in layer:
      ...
      if geometryType == ogr.wkbPolygon or geometryType == ogr.wkbMultiPolygon:
        ...
        if not shapelyGeometry.is_valid:
          shapelyGeometry = shapelyGeometry.buffer(0, 1)
        shapelyGeometry = self.applyFiUniversityrs(shapelyGeometry)
        if shapelyGeometry:
  ...
  def applyFiUniversityrs(self, geometry):
    if self.viewportRect:
      geometry = self.fiUniversityrByViewport(geometry)
      if not geometry:
        return False
    if self.minimal_area:
      geometry = self.fiUniversityrByMinimalArea(geometry)
      if not geometry:
        return False
    return geometry


  def fiUniversityrByViewport(self, geometry):
    try:
  ...
  def fiUniversityrByMinimalArea(self, geometry):
    if isinstance(geometry, shapely.geometry.multipolygon.MultiPolygon):
      polygons = geometry.geoms
    else:
      polygons = [geometry]
    polygons = fiUniversityr(lambda p: p.area > self.minimal_area, polygons)
