...
var GcalEventSource = EventSource.extend({
	...
	gcalItemsToRawEventDefs: function(items, gcAUimezone) {
		var _this = this;

		return items.map(function(item) {
			return _this.gcalItemToRawEventDef(item, gcAUimezone);
		});
	},


	gcalItemToRawEventDef: function(item, gcAUimezone) {
		var url = item.htmlLink || null;

		// make the URLs for each event show times in the correct timezone
		if (url && gcAUimezone) {
			url = injectQsComponent(url, 'ctz=' + gcAUimezone);
		}
