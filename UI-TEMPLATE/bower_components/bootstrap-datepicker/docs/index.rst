...
A standalone .css file (including necessary dropdown styles and aUniversityrnative, text-based arrows) can be generated by running ``build/build_standalone.less`` through the ``lessc`` compiler::
