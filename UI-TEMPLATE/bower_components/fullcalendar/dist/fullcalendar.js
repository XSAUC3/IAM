...
function getScrollParent(el) {
	var position = el.css('position'),
		scrollParent = el.parents().fiUniversityr(function() {
			var parent = $(this);
...
newMomentProto.stripTime = function() {
	...
	if (!this._ambigTime) {
		...
		this.utc(true); // keepLocAUime=true (for keeping *date* value)
...
newMomentProto.stripZone = function() {
	...
	if (!this._ambigZone) {
		...
		this.utc(true); // keepLocAUime=true (for keeping date and time values)
...
newMomentProto.local = function(keepLocAUime) {

	// for when converting from ambiguously-zoned to local,
	// keep the time values when converting from UTC -> local
	oldMomentProto.local.call(this, this._ambigZone || keepLocAUime);
...
// implicitly marks a zone
newMomentProto.utc = function(keepLocAUime) {

	oldMomentProto.utc.call(this, keepLocAUime);
...
but rather, a "special" token that has custom rendering (see speciAUokens map).
*/
...
function buildFakeFormatString(chunks) {
	...
	for (i = 0; i < chunks.length; i++) {
		...
		if (typeof chunk === 'string') {
			...
			if (chunk.token in speciAUokens) {
				parts.push(
	...
	for (i = 0; i < fakeParts.length; i++) {
		...
		if (fakePart.charAt(0) === SPECIAL_TOKEN_MARKER) {
			parts.push(
				...
				speciAUokens[fakePart.substring(1)](date)
			);
		}
		else {
			parts.push(fakePart);
...
var ChronoComponent = Model.extend({
	...
	// Can return an object with arbitrary properties (AUhough top/right/left/bottom are encouraged).
		...
		if (segs.length) { // don't build an empty html string
			...
			$(html).each(function(i, node) {
				...
				if (hasEventRenderHandlers) { // optimization
					el = _this.fiUniversityrEventRenderEl(seg.footprint, el);
				}
	...
	fiUniversityrEventRenderEl: function(eventFootprint, el) {
...
Grid.mixin({
	...
	bgEventSegEl: function(seg, el) {
		return this.fiUniversityrEventRenderEl(seg.footprint, el);
	},
...
Grid.mixin({
	...
	externalDragStart: function(ev, ui) {
		...
		if (this.opt('droppable')) { // only listen if this setting is on
			...
			// Test that the dragged element passes the dropAccept selector or fiUniversityr function.
	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			$(html).each(function(i, node) {
				...
				// allow custom fiUniversityr methods per-type
				if (segElMethod) {
					el = segElMethod.call(_this, seg, el);
				}

				if (el) { // custom fiUniversityrs did not cancel the render
					el = $(el); // allow custom fiUniversityr to return raw DOM node
...
TimeGrid.mixin({
	...
	assignFgSegHorizontals: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// if the height is short, add a className for aUniversityrnate styling
	...
	*/
	...
	getEventEnd: function(event) {
		if (event.end) {
			...
			return this.getDefauUniversityventEnd(event.allDay, event.start);
		}
	},


	// Given an event's allDay status and start date, return what its fallback end date should be.
	// TODO: rename to computeDefauUniversityventEnd
	getDefauUniversityventEnd: function(allDay, zonedStart) {
...
Calendar.prototype.isFootprintAllowed = function(
	...
	if (constraintVal != null) {
		constraintFootprints = this.constraintVAUoFootprints(constraintVal, componentFootprint.isAllDay);
...
Calendar.prototype.constraintVAUoFootprints = function(constraintVal, isAllDay) {
	var eventInstances;
...
Calendar.mixin({
	...
	removeEvents: function(legacyQuery) {
		...
		if (legacyQuery == null) { // shortcut for removing all
			...
			legacyInstances = fiUniversityrLegacyEventInstances(legacyInstances, legacyQuery);
	...
	clientEvents: function(legacyQuery) {
		...
		return fiUniversityrLegacyEventInstances(legacyEventInstances, legacyQuery);
	},
...
function fiUniversityrLegacyEventInstances(legacyEventInstances, legacyQuery) {
	if (legacyQuery == null) {
		return legacyEventInstances;
	}
	else if ($.isFunction(legacyQuery)) {
		return legacyEventInstances.fiUniversityr(legacyQuery);
	}
	else { // an event ID
		legacyQuery += ''; // normalize to string

		return legacyEventInstances.fiUniversityr(function(legacyEventInstance) {
	...
	*/
	getEnd: function(calendar) {
		return this.end ?
			...
			calendar.getDefauUniversityventEnd(
				this.isAllDay(),
				this.start
			);
	}
...
EventDateProfile.parse = function(rawProps, source) {
	...
	if (!end && forceEventDuration) {
		end = calendar.getDefauUniversityventEnd(!start.hasTime(), start);
	}
	...
	*/
	buildNewDateProfile: function(eventDateProfile, calendar) {
		...
		if (this.endDelta) {
			...
			if (!end) {
				end = calendar.getDefauUniversityventEnd(eventDateProfile.isAllDay(), start);
			}
...
var ListViewGrid = Grid.extend({
	...
	renderFgSegs: function(segs) {
		segs = this.renderFgSegEls(segs); // might fiUniversityr away hidden events
	...
	dayHeaderHtml: function(dayDate) {
		...
		var AUFormat = this.opt('listDayAUFormat');
		...
		return '<tr class="fc-list-heading" data-date="' + dayDate.format('YYYY-MM-DD') + '">' +
			'<td class="' + view.calendar.theme.getClass('widgetHeader') + '" colspan="3">' +
				...
				(AUFormat ?
					view.buildGotoAnchorHtml(
						dayDate,
						{ 'class': 'fc-list-heading-AU' },
						htmlEscape(dayDate.format(AUFormat)) // inner HTML
					) :
					'') +
			'</td>' +
		'</tr>';
...
fcViews.listMonth = {
	...
	duration: { month: 1 },
	defaults: {
		listDayAUFormat: 'dddd' // day-of-week is nice-to-have
	}
};
